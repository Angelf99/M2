...
spn.innerHTML = spn.innerHTML +' Angel'
...
function buildToDos(toDos) {
  // Tu código acá://map y filter devuelven un nuevo arrray
...
function displayToDos() {
  // Tu código acá:
  let conteiner= document.querySelector('#toDoContainer');
  conteiner.innerHTML='';

  let build = buildToDos(toDoItems);
  for (let i=0 ; i<build.length; i++){
    conteiner.appendChild(build[i])
  }
}
